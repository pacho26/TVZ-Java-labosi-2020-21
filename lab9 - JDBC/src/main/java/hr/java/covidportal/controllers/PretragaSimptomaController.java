...
public class PretragaSimptomaController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        List<Simptom> procitaniSimptomi = new ArrayList<>();
        try {
            procitaniSimptomi = BazaPodataka.dohvacanjeSvihSimptoma();
        }
        catch (SQLException throwables) {
            throwables.printStackTrace();
        }
        procitaniSimptomi = procitaniSimptomi.stream()
                .sorted(Comparator.comparing(Simptom::getNaziv))
