...
import hr.java.covidportal.genericsi.VirusGenericLabos;
...
public class Glavna {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("\nFiltrirani virusi: ");
        virusi.stream()
                .filter(v -> v.getNaziv().toLowerCase().startsWith("c") && v.getNaziv().toLowerCase().contains("19"))
                .sorted(Comparator.comparing(ImenovaniEntitet::getNaziv))
                .forEach(System.out::println);

        //-------------------------------2. ZADATAK-------------------------------
        System.out.println("\nSuma duljina virusa: " + virusi.stream()
                .mapToInt(Virus -> Virus.getNaziv().length()).sum());

        System.out.println("Prosjek duljina virusa: " + virusi.stream()
                .mapToDouble(Virus -> Virus.getNaziv().length()).average().getAsDouble());

        System.out.println("Duljina virusa s najkraćim nazivom: " + virusi.stream()
                .mapToInt(Virus -> Virus.getNaziv().length()).min().getAsInt());

        System.out.println("Duljina virusa s najduljim nazivom: " + virusi.stream()
                .mapToInt(Virus -> Virus.getNaziv().length()).max().getAsInt());

        //-------------------------------3.ZADATAK-------------------------------
        VirusGenericLabos<Virus> sviVirusiLabos = new VirusGenericLabos<>();
        for (Bolest bolest : bolesti) {
            if (bolest instanceof Virus) {
                sviVirusiLabos.add((Virus) bolest);
            }
        }
        System.out.println("\nIspis svih virusa pomoću generičke liste: ");
        sviVirusiLabos.getVirusiGenerics().stream()
                .forEach(System.out::println);

        //primjer dohvaćanja nekog elementa liste pomoću indeksa i metode get u generičkoj listi
        System.out.println("\nElement na indeksu [0]: " + sviVirusiLabos.get(0));

        //metoda po želji
        sviVirusiLabos.brisiPosljednjeg();
        System.out.println("\nLista nakon brisanja posljednjeg elementa: ");
        sviVirusiLabos.getVirusiGenerics().stream()
                .forEach(System.out::println);
    ...
    private static void MapiranjeVirusaIBrojaSimptoma(Set<Bolest> bolesti) {
    ...
    private static void SortiranjeMjerenjeIspisVirusa(List<Virus> virusi) {
        ...
        System.out.println("Proteklo vrijeme(LAMBDA): " + Duration.between(start, end).toMillis() + " ms");

        start = Instant.now();
        virusi.sort(new VirusSorter());
        end = Instant.now();
        System.out.println("Proteklo vrijeme(COMPARATOR): " + Duration.between(start, end).toMillis() + " ms");
    }
     ...
     */
    private static void PopunjavanjeMapeBolesti(Set<Bolest> bolesti, List<Osoba> osobe,
